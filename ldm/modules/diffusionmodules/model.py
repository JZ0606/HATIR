...
class StructureAttention(nn.Module):
    ...
    def __init__(self, channels, lambda_coef=0.1):
        super().__init__()
    ...
    def compute_gradient_magnitude(self, x):
        ...
        grad_x = F.conv2d(x_reshaped, self.sobel_x, padding=1, stride=1)
        grad_y = F.conv2d(x_reshaped, self.sobel_y, padding=1, stride=1)
        

        channel_gradient_mag = torch.sqrt(grad_x**2 + grad_y**2 + 1e-8)  

        channel_gradient_mag = channel_gradient_mag.view(t, c, h, w)
        gradient_mag_total = channel_gradient_mag.sum(dim=1, keepdim=True) 
        
        return gradient_mag_total
    
    def forward(self, f_t):

        gradient_l1_norm = self.compute_gradient_magnitude(f_t)

        A_t = self.attention_generator(gradient_l1_norm)

        f_t_enh = f_t + self.lambda_coef * (f_t * A_t.expand_as(f_t))
...
class VideoDecoder_Mix(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        # z to block_in
        self.conv_in_1 = torch.nn.Conv2d(z_channels,
                                       block_in,
                                       kernel_size=3,
                                       stride=1,
                                       padding=1)
        
        self.resnet_in = ResnetBlock(in_channels=block_in,
                                       out_channels=block_in,
                                       temb_channels=self.temb_ch,
                                       dropout=dropout)
        
        self.conv_in_2 = torch.nn.Conv2d(block_in,
                                       block_in,
                                       kernel_size=1,
                                       stride=1,
                                       padding=0)
        
        self.conv_g = torch.nn.Conv2d( 1,
                                       block_in,
                                       kernel_size=1,
                                       stride=1,
                                       padding=0)
        ...
        self.struct_attn = StructureAttention(block_in)
        # middle
        self.mid = nn.Module()
        self.mid.block_1 = ResnetBlock(in_channels=block_in,
                                       out_channels=block_in,
    ...
    def forward(self, z, enc_fea, flows, phasors):
        #assert z.shape[1:] == self.z_shape[1:]
        self.last_z_shape = z.shape
        ...
        # z to block_in
        h_in = self.conv_in_1(z)
        h_in = self.resnet_in(h_in, temb)
        h_in = self.conv_in_2(h_in)
        ...
        if flow_fwd.dim() == 5:
            flow_fwd = rearrange(flow_fwd, 'b t c h w -> (b t) c h w', t=t-1)
            flow_bwd = rearrange(flow_bwd, 'b t c h w -> (b t) c h w', t=t-1)

        if flow_fwd.shape[2] != h_in.shape[2] or flow_fwd.shape[3] != h_in.shape[3]:
            flow_fwd = F.interpolate(flow_fwd, size=(h, w), mode='bilinear', align_corners=True)
            flow_bwd = F.interpolate(flow_bwd, size=(h, w), mode='bilinear', align_corners=True)

        # phasors: t, 1, h, w
        # flows: t-1, 2, h, w

        
        h_in_fwd[1:, :, :, :] = flow_warp(h_in[1:, :, :, :],flow_fwd.permute(0, 2, 3, 1) )
        h_in_bwd[:t-1, :, :, :] = flow_warp(h_in[:t-1, :, :, :].flip(0),flow_bwd.permute(0, 2, 3, 1) )
            
        turbulence_map = torch.norm(h_in_fwd - h_in, p=1, dim=1, keepdim=True) + torch.norm(h_in_bwd - h_in, p=1, dim=1, keepdim=True)
        # t: t, 1, h, w
        
        gating_mask = self.sigmoid(self.conv_g(turbulence_map)) 
        h = h_in * gating_mask
