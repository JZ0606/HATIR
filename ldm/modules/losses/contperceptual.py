...
def l1_thermal(x, y, phasors,t):
    bt, _, _, _ = x.shape
    x = rearrange(x, '(b t) c h w -> b t c h w', t=t)
    y = rearrange(y, '(b t) c h w -> b t c h w', t=t)
    
    out = rearrange(phasors * (x - y), 'b t c h w -> (b t) c h w')
    return torch.abs(out)

def l1_edge(x, y, phasors, t):
    bt, c, h, w = x.shape
    ...
    y_edge = torch.zeros_like(y)
    
    for b_idx in range(b):
        for t_idx in range(t):
            x_batch = x[b_idx, t_idx]  # [c, h, w]
            y_batch = y[b_idx, t_idx]  # [c, h, w]
            
            x_batch = x_batch.unsqueeze(0)  # [1, c, h, w]
            y_batch = y_batch.unsqueeze(0)  # [1, c, h, w]
            
            laplacian_kernel_expanded = laplacian_kernel.repeat(c, 1, 1, 1)  # [c, 1, 3, 3]
            
            x_edge_batch = F.conv2d(x_batch, laplacian_kernel_expanded, padding=1, groups=c)
            y_edge_batch = F.conv2d(y_batch, laplacian_kernel_expanded, padding=1, groups=c)
            
            x_edge[b_idx, t_idx] = x_edge_batch.squeeze(0)
            y_edge[b_idx, t_idx] = y_edge_batch.squeeze(0)
    
    edge_diff = x_edge - y_edge
    
    if phasors.dim() == 4:  
        phasors = phasors.unsqueeze(2)  
    if phasors.shape[2] == 1:  
        phasors = phasors.expand(-1, -1, c, -1, -1)
    
    weighted_diff = phasors * edge_diff
    
    out = rearrange(weighted_diff, 'b t c h w -> (b t) c h w')
...
class LPIPSWithDiscriminator(nn.Module):
    def __init__(self, disc_start, logvar_init=0.0, kl_weight=1.0, pixelloss_weight=1.0,
                 diffloss_weight=1.0, thermalloss_weight=1.0, edgeloss_weight=1.0, temploss_weight=1.0,
        ...
        # loss
        self.thermalloss_weight = thermalloss_weight
        self.edgeloss_weight = edgeloss_weight
    ...
    def forward(self, inputs, reconstructions, posteriors, phasors, optimizer_idx,
        ...
        if phasors.shape[2] != inputs.shape[2] or phasors.shape[3] != inputs.shape[3]:
            phasors = F.interpolate(phasors, size=(inputs.shape[2], inputs.shape[3]), mode='bilinear', align_corners=True)

        # thermal reconstruction loss
        thermal_loss = l1_thermal(inputs.contiguous(), reconstructions.contiguous(), phasors=phasors, t=self.num_frames)
        weighted_nll_loss += self.thermalloss_weight * torch.mean(thermal_loss) / thermal_loss.shape[0]
        # edge loss
        edge_loss = l1_edge(inputs.contiguous(), reconstructions.contiguous(), phasors=phasors, t=self.num_frames)
        weighted_nll_loss += self.edgeloss_weight * torch.mean(edge_loss) / edge_loss.shape[0]
        # diff loss
