    ...
    const int dilation_w, const int attn_head, const int deform_group, const int clip_size)
{
  if (q.device().is_cuda())
  {
    ...
    return deform_attn_cuda_forward(q, kv,
                                    offset, phasor, output, columns, attns, mask_ones, kernel_h, kernel_w, stride_h,
...
void deform_attn_backward(
    at::Tensor q, at::Tensor kv, at::Tensor offset, at::Tensor phasor, at::Tensor columns,
    ...
    int pad_w, int dilation_h, int dilation_w, int attn_head, int deform_group, int clip_size)
{
  if (q.device().is_cuda())
  {
    ...
    return deform_attn_cuda_backward(q, kv,
                                     offset, phasor, columns, attns, mask_ones, grad_attns, grad_mask_ones, grad_q, grad_kv, grad_offset,
