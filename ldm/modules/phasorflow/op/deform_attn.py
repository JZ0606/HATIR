...
class DeformAttnFunction(Function):
    ...
    def forward(ctx,
        ...
        phasor = phasor.flatten(0, 1).flatten(1, 2).unsqueeze(1).unsqueeze(3).unsqueeze(4) # [Bn, 1, HW, 1, 1]
        # print("phasor 1", phasor.shape) 
        ctx.phasor = phasor.repeat(1, attention_heads, 1, 1, clip_size*attn_size)
        # b x (attn_head x (height*width) x 1 x (clip_size*attn_size))
        ...
        deform_attn_ext.deform_attn_forward(q, kv, offset, ctx.phasor, output,
    ...
    def backward(ctx, grad_output):
        ...
        deform_attn_ext.deform_attn_backward(q, kv, offset, ctx.phasor, ctx._bufs[0], ctx._bufs[1], ctx._bufs[2], ctx._bufs[3], ctx._bufs[4],
                                                       grad_q, grad_kv, grad_offset,
                                                       grad_output, ctx.kernel_h, ctx.kernel_w, ctx.stride,
                                                       ctx.stride, ctx.padding, ctx.padding, ctx.dilation, ctx.dilation,
                                                       ctx.attention_heads, ctx.deformable_groups, ctx.clip_size)

        return (grad_q, grad_kv, grad_offset, None, None, None, None, None, None, None, None, None)
