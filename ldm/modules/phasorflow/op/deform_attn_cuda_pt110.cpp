...
void deform_attn_cuda_forward(
    at::Tensor q, at::Tensor kv, at::Tensor offset, at::Tensor phasor, at::Tensor output,
    ...
    const int dilation_w, const int attn_head, const int deform_group, const int clip_size)
{
    ...
    { // 0->2,1->2, or, 1->3,0->3 // todo: refer to deformable_im2col_cuda and use `im2col_step` to speed up
        ...
        // std::cout << "q[" << b << "] shape: ";
        // auto q_b = q[b];
        // for (int i = 0; i < q_b.dim(); ++i)
        // {
        //     std::cout << q_b.size(i) << " ";
        // }
        // std::cout << std::endl;

        // std::cout << "columns[0] shape: ";
        // auto columns_0 = columns[0];
        // for (int i = 0; i < columns_0.dim(); ++i)
        // {
        //     std::cout << columns_0.size(i) << " ";
        // }
        // std::cout << std::endl;

        // std::cout << "phasor[b] shape: ";
        // auto phasor_0 = phasor[0];
        // for (int i = 0; i < phasor_0.dim(); ++i)
        // {
        //     std::cout << phasor_0.size(i) << " ";
        // }
        // std::cout << std::endl;


        attns = phasor[b] * at::matmul(q[b], columns[0]);

        // std::cout << "attns shape: ";
        // for (int i = 0; i < attns.dim(); ++i)
        // {
        //     std::cout << attns.size(i) << " ";
        // }
        // std::cout << std::endl;

        // attns = at::matmul(q[b], columns[0]);

        attns = attns.softmax(-1); // (attn_head x (height*width) x 1 x (clip_size*attn_size))
        // do attention
...
void deform_attn_cuda_backward(
    at::Tensor q, at::Tensor kv, at::Tensor offset, at::Tensor phasor,
