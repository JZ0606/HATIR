...
class LatentDiffusionVSRTextWT(DDPM):
    ...
    def compute_temporal_condition_v1(self, lr_images, latents):
        ...
        for i in range(0, t):
            ...
            if i > 0:  # no warping required for the first timestep
                ...
                latent_curr_warp = flow_warp(latent_curr, flow)
    ...
    def compute_temporal_condition_v4(self, flows, latents, masks, phasors):
        ...
        if phasors.shape[3] != latents.shape[3] or phasors.shape[4] != latents.shape[4]:
            phasors = rearrange(phasors, 'b t 1 h w -> (b t) 1 h w', t=t)
            
            phasors = F.interpolate(
                phasors, 
                size=(latents.shape[3], latents.shape[4]),
                mode='bilinear', 
                align_corners=False
            )
            phasors = rearrange(phasors, '(b t) 1 h w -> b t 1 h w', t=t)
        ...
        for i in range(t - 1, -1, -1):
            ...
            if i < t - 1:  # no warping required for the last timestep
                ...
                loss_b += F.l1_loss(phasors[:, i, :, :, :] * (1 - fwd_occs[:, i, :, :, :]) * latent_prev, phasors[:, i, :, :, :] * (1 - fwd_occs[:, i, :, :, :]) * latent_curr)
            latent_prev = latent_curr_warp
        loss_f = 0
        latent_curr_warp = torch.zeros_like(latents[:, 0, :, :, :])
        # forward propagation
        for i in range(0, t):
            ...
            if i > 0:  # no warping required for the first timestep
                ...
                latent_curr_warp = flow_warp(latent_curr, flow.permute(0, 2, 3, 1), interp_mode='bilinear')
                loss_f += F.l1_loss(phasors[:, i-1, :, :, :] * (1 - bwd_occs[:, i-1, :, :, :]) * latent_prev, phasors[:, i-1, :, :, :] * (1 - bwd_occs[:, i-1, :, :, :]) * latent_curr)
            latent_prev = latent_curr_warp
        return loss_b + loss_f
    ...
    def p_sample(self, x, c, struct_cond, t, guidance_scale=-1.0,
                 lr_images=None, flows=None, phasors=None, masks=None, clip_denoised=False, repeat_noise=False,
                 return_codebook_ids=False, quantize_denoised=False, return_x0=False,
                 temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None, t_replace=None):
        b, *_, device = *x.shape, x.device
        b = b // self.num_frames
        ...
        if flows is not None:
            with torch.enable_grad():
                latents = latents.detach()
                latents.requires_grad = True
                loss_tempo = self.compute_temporal_condition_v4(flows, latents, masks, phasors)
                latents = latents - guidance_scale * model_log_variance * torch.autograd.grad(loss_tempo, latents)[0]
                latents = latents.detach()
                # latents.requires_grad = False
        if return_codebook_ids:
    ...
    def p_sample_loop(self, cond, struct_cond, shape, guidance_scale=-1.0,
                      lr_images=None, flows=None, phasors=None, masks=None, return_intermediates=False,
                      x_T=None, verbose=True, callback=None, timesteps=None, quantize_denoised=False,
                      mask=None, x0=None, img_callback=None, start_T=None,
                      log_every_t=None, time_replace=None, adain_fea=None, interfea_path=None):
        ...
        for i in iterator:
            ...
            img = self.p_sample(img, cond, struct_cond_input, ts, guidance_scale=guidance_scale,
                                lr_images=lr_images, flows=flows, phasors=phasors, masks=masks,
                                clip_denoised=self.clip_denoised,
                                quantize_denoised=quantize_denoised, t_replace=t_replace)
    ...
    def sample(self, cond, struct_cond, guidance_scale=-1.0,
               lr_images=None, flows=None, phasors=None, masks=None, batch_size=16, return_intermediates=False, x_T=None,
               verbose=True, timesteps=None, quantize_denoised=False,
               mask=None, x0=None, shape=None, time_replace=None, adain_fea=None, interfea_path=None, start_T=None, **kwargs):
