...
class VideoAutoencoderKLResi(pl.LightningModule):
    ...
    def decode(self, z, enc_fea, flows, phasors):
        z = self.post_quant_conv(z)
        dec = self.decoder(z, enc_fea, flows, phasors)
        return dec

    def forward(self, input, latent, flows, phasors, sample_posterior=True):
        posterior, enc_fea_lq = self.encode(input)
        dec = self.decode(latent, enc_fea_lq, flows, phasors)
        return dec, posterior
    ...
    def get_input(self, batch):
        ...
        gt = gt.to(memory_format=torch.contiguous_format).float()
        latent = latent.to(memory_format=torch.contiguous_format).float() / 0.18215
        phasor = phasor.to(memory_format=torch.contiguous_format).float()
        flows[0] = flows[0].to(memory_format=torch.contiguous_format).float()
        flows[1] = flows[1].to(memory_format=torch.contiguous_format).float()
        ...
        gt = rearrange(gt, 'b t c h w -> (b t) c h w')
        latent = rearrange(latent, 'b t c h w -> (b t) c h w')
        phasor = rearrange(phasor, 'b t c h w -> (b t) c h w')
        flows[0] = rearrange(flows[0], 'b t c h w -> (b t) c h w')
        flows[1] = rearrange(flows[1], 'b t c h w -> (b t) c h w')

        return input, gt, latent, phasor, flows
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        if self.synthesis_data:
            ...
            inputs, gts, latents, phasors, flows = self.get_input(batch)
        reconstructions, posterior = self(inputs, latents, flows, phasors)

        if optimizer_idx == 0:
            # train encoder+decoder+logvar
            aeloss, log_dict_ae = self.loss(gts, reconstructions, posterior, phasors, optimizer_idx, self.global_step,
                                            last_layer=self.get_last_layer(), split="train")
            self.log("aeloss", aeloss, prog_bar=True, logger=True, on_step=True, on_epoch=True)
            self.log_dict(log_dict_ae, prog_bar=False, logger=True, on_step=True, on_epoch=False)
            return aeloss
        ...
        if optimizer_idx == 1:
            # train the discriminator
            discloss, log_dict_disc = self.loss(gts, reconstructions, posterior, phasors, optimizer_idx, self.global_step,
                                                last_layer=self.get_last_layer(), split="train")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        reconstructions, posterior = self(inputs, latents, flows, phasors)
        aeloss, log_dict_ae = self.loss(gts, reconstructions, posterior, phasors, 0, self.global_step,
                                        last_layer=self.get_last_layer(), split="val")

        discloss, log_dict_disc = self.loss(gts, reconstructions, posterior, phasors, 1, self.global_step,
                                            last_layer=self.get_last_layer(), split="val")
    ...
    def log_images(self, batch, only_inputs=False, **kwargs):
        log = dict()
        if self.synthesis_data:
            x, gts, latents, _ = self.get_input_synthesis(batch, val=False)
        else:
            x, gts, latents, phasors, flows = self.get_input(batch)
        x = x.to(self.device)
        latents = latents.to(self.device)
        if not only_inputs:
            xrec, posterior = self(x, latents, flows, phasors)
            if x.shape[1] > 3:
                # colorize with random projection
                assert xrec.shape[1] > 3
                x = self.to_rgb(x)
