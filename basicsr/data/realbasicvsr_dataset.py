...
from basicsr.data.mmcv_transforms import RandomBlur, RandomResize, RandomNoise, RandomJPEGCompression, RandomVideoCompression, adjust_sharpness
...
class RealVSRRecurrentDataset(data.Dataset):
    ...
    Args:
        ...
        val_partition (str): Validation partition types. 'REDS4' or 'official' or 'HATIR'.
    ...
    def __init__(self, opt):
        ...
        if opt['val_partition'] == 'REDS4':
            ...
            val_partition = [f'{v:04d}' for v in range(240, 270)]
            if opt['test_mode']:
                self.keys = [v for v in self.keys if v.split('/')[0] in val_partition]
            else:
                self.keys = [v for v in self.keys if v.split('/')[0] not in val_partition]
    ...
    def __getitem__(self, index):
        ...
        # randomly crop
        img_gts, img_lqs = paired_random_crop(img_gts, img_lqs, gt_size, 4, img_gt_path)
        ...
        if self.opt['test_mode']:
            ...
            img_gts = torch.stack(img_results[len(img_lqs) // 2:], dim=0).numpy()
            img_lqs = torch.stack(img_results[:len(img_lqs) // 2], dim=0).numpy()

            out_dict = {'lqs': np.transpose(img_lqs, (0, 3, 2, 1)), 'gts': np.transpose(img_gts, (0, 3, 2, 1))}
            out_dict['gts'] = [adjust_sharpness(img, factor=2.0) for img in out_dict['gts']]
            # out_dict = {'lqs': img_lqs, 'gts': img_gts}
                
        # list-to-list
        for k in out_dict.keys():
            if isinstance(out_dict[k], list) == False:
                t = out_dict[k].shape[0]
                out_dict[k] = np.split(out_dict[k], t, axis=0)
                out_dict[k] = [img.squeeze(axis=0) for img in out_dict[k]]
                # print(k, len(out_dict[k]), out_dict[k][0].shape)
            
                
            out_dict[k] = img2tensor(out_dict[k])  
            for img in out_dict[k]:
                if img.shape[0] == 3:
    ...
    def __init__(self, opt):
        ...
        with open(opt['meta_info_file'], 'r') as fin:
            for line in fin:
                ...
                self.keys.extend([f'{folder}/{i:03d}' for i in range(int(frame_num))])
