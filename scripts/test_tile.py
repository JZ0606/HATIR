...
def main():
    ...
    parser.add_argument(
        "--phasorflow_ckpt",
        type=str,
        help="path to checkpoint of PhasorFlow model",
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    vqgan_config = OmegaConf.load("configs/TAD/tad.yaml")
    vq_model = load_model_from_config(vqgan_config, opt.vqgan_ckpt)
    vq_model = vq_model.to(device)
    vq_model.decoder.fusion_w = opt.dec_w
    
    phasorflow_config = OmegaConf.load("configs/PhasorFlow/phasorflow.yaml")
    phasorflow = load_model_from_config(phasorflow_config, opt.phasorflow_ckpt)
    phasorflow = phasorflow.to(device)
            ...
            with precision_scope("cuda"):
                with model.ema_scope():
                    ...
                    for n in trange(len(init_segment_list), desc="Sampling"):
                        ...
                        # flow estimation
                        im_lq_bs_0_1 = F.interpolate(im_lq_bs_0_1, size=(im_h//2, im_w//2), mode='bicubic')
                        im_lq_bs_0_1 = rearrange(im_lq_bs_0_1, '(b t) c h w -> b t c h w', t=init_image.size(0))
                        flows, phasors = phasorflow.compute_phasorflow(im_lq_bs_0_1)
                        ...
                        phasors = rearrange(phasors, 'b t c h w -> (b t) c h w')
                        ...
                        if im_lq_bs.shape[2] > opt.vqgantile_size or im_lq_bs.shape[3] > opt.vqgantile_size:
                            ...
                            phasors_spliter = ImageSpliterTh(phasors, opt.vqgantile_size // 8, opt.vqgantile_stride // 8, sf=1)

                            for (im_lq_pch, index_infos), (flow_f, _), (flow_b, _), (fwd_occ, _), (bwd_occ, _), (phasors, _) \
                                in zip(imlq_spliter, flow_spliter_f, flow_spliter_b, fwd_occ_spliter, bwd_occ_spliter, phasors_spliter):
                                seed_everything(opt.seed)
                                
                                init_latent_generator, enc_fea_lq = vq_model.encode(im_lq_pch)
                                ...
                                phasors = rearrange(phasors, '(b t) c h w -> b t c h w', t=init_image.size(0))
                                ...
                                x_samples = vq_model.decode(samples * 1. / model.scale_factor, enc_fea_lq, flows, phasors)
