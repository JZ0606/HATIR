            ...
            with precision_scope("cuda"):
                with model.ema_scope():
                    ...
                    for n in trange(len(init_segment_list), desc="Sampling"):
                        ...
                        flows, phasors = phasorflow.compute_phasorflow(init_image_0_1)
                        ...
                        os.makedirs(os.path.join(opt.phasor_dir, seq_item), exist_ok=True)
                        os.makedirs(os.path.join(opt.flow_dir, seq_item), exist_ok=True)
                        for i in range(init_image.size(0)):
                            ...
                            with open(os.path.join(opt.phasor_dir, seq_item, basename + '.npy'), 'wb') as f:
                                np.save(f, phasors[0][i].cpu().numpy())
                            if i >= 1:
                                with open(os.path.join(opt.flow_dir, seq_item, basename + '_fwd.npy'), 'wb') as f:
                                    np.save(f, flows[0][0][i - 1].cpu().numpy())
                            if i < init_image.size(0) - 1:
                                with open(os.path.join(opt.flow_dir, seq_item, basename + '_bwd.npy'), 'wb') as f:
                                    np.save(f, flows[1][0][i].cpu().numpy())
                    ender.record()
                    toc = time.time()
                    # WAIT FOR GPU SYNC
